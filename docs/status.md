...
Split out of [`AGENTS.md`](../AGENTS.md), which carries the rules an agent must
follow on every change. This file carries the record of what is built and what
is not — read it before claiming something works, and update it in the same
...
## Phase 2C — where it stands

Slices 0, 1 and 3 are merged. Slices 2 and 4-6 follow from one settled product
decision: **the portfolio starts empty and an operator adds clients.** The
alternative — seeding the eight fixture clients from the old `data.ts` as real
rows — is a faster demo that puts invented client names in a real database,
which is the exact thing this phase exists to remove. Starting empty also makes
the first-run state the normal state rather than a screen nobody sees until the
first real customer.
