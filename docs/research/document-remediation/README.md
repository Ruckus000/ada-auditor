...
| **[source-fidelity-in-production.md](source-fidelity-in-production.md)** | **The delivered document, held against its own source.** Nothing compared a delivered PDF to the `.docx` it came from — the existing gate compares a PDF with itself. Re-measured on master 2026-09-11: 31/31 converted, **0 assertions**, 24 fully faithful. Found content loss nothing else could see — five list items dropped by the export on three documents, and one document losing all six of its list items — and, on re-measurement, that our own standing heading re-rank shows up as a divergence from the source. |
