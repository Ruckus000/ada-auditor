...
| **[source-fidelity-in-production.md](source-fidelity-in-production.md)** | **The delivered document, held against its own source — in production, on every conversion.** 31/31 real Word documents, 0 assertions, 26 fully faithful. Found four defects in its own first run (including a `\b` regex that counted table cells as tables, still live in `extract-docx-truth.mjs`) and one real content loss nobody could see before: five list items dropped by the PDF export on three documents. |
