...
| `AUDITOR_STORE` | No | Set to `memory` to run against ephemeral in-process stores instead of Postgres. For harnesses that boot the real server without a database — the hydration and accessibility suites in CI. It is an explicit opt-in, never a fallback for a missing `DATABASE_URL`: a fallback would let a misconfigured deploy serve an empty client list and discard every run silently. Nothing persists, and the process logs `store_memory_mode` on the way past. Never set it in a deployed environment. |
...
| `AUDITOR_HTMLCS_TIMEOUT_MS` | No | How long the HTML_CodeSniffer second-opinion scan may run per page, in milliseconds. Default 15000 (`DEFAULT_HTMLCS_TIMEOUT_MS` in `src/domain/run-limits.ts`). It shares the walk budget with the primary engine and can never gate a run, so on expiry the page keeps its axe results and simply has no second opinion (`htmlcs_scan_unavailable` in the logs) — a hung extra scanner must not starve the one that decides. |
