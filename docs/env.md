...
| `AUDITOR_OPERATOR_NAME` | No | Name that activity is attributed to, and the name the platform header shows. There is no per-user identity in this product — one shared token, one trusted operator group — so this is a name rather than an account, and `activity_events.actor` stores it as text rather than a foreign key. Unset renders as `Operator`. It replaced a hardcoded `"Jules Reyes"` that read as though the product knew who was signed in. |
