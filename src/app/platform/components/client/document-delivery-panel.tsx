    ...
    <dialog ref={dialog} aria-labelledby={titleId} onCancel={event => { event.preventDefault(); if (!busy) close(); }} style={{ width: 'min(640px, calc(100vw - 40px))', maxHeight: '85vh', overflow: 'auto', border: `1px solid ${T.rule}`, borderRadius: 14, padding: 24, color: T.ink, background: T.surface }}>
      {modal ? <><h3 id={titleId} style={{ marginTop: 0 }}>{modal.kind === 'bundle' ? 'Review the delivery bundle' : modal.kind === 'signoff' ? 'Sign off this output' : 'Exclude this document'}</h3>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 18, flexWrap: 'wrap' }}><button type="button" style={buttonStyle} disabled={busy} onClick={close}>Close</button>
          {modal.kind === 'bundle' ? !modal.bundle.issuedAt ? <button type="button" style={buttonStyle} disabled={busy} onClick={() => void act(`/delivery/${encodeURIComponent(modal.bundle.id)}`, { action: 'issue' })}>Issue delivery link</button> : !modal.bundle.revokedAt ? <button type="button" style={buttonStyle} disabled={busy} onClick={() => void act(`/delivery/${encodeURIComponent(modal.bundle.id)}`, { action: 'revoke' })}>Revoke link</button> : null : <button type="button" style={buttonStyle} disabled={busy || (modal.kind === 'exclude' && !answer.trim())} onClick={() => void act(`/documents/${encodeURIComponent(modal.row.documentId)}/${modal.kind === 'signoff' ? 'signoff' : 'exclusion'}`, modal.kind === 'signoff' ? { ...(answer.trim() ? { note: answer.trim() } : {}) } : { reason: answer.trim() })}>{busy ? 'Working…' : modal.kind === 'signoff' ? 'Verify and sign off' : 'Record exclusion'}</button>}
        </div>
      </> : null}
    </dialog>
  </section>;
