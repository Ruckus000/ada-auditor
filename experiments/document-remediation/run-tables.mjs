...
const [IN = 'out/e2-headings', OUT = 'out/e2-tables'] = process.argv.slice(2);
...
for (const f of readdirSync(IN).filter((x) => x.endsWith('.pdf')).sort()) {
  ...
  try {
    const out = execFileSync(`${JAVA_HOME}/bin/java`,
      ['-Djava.awt.headless=true', '-cp', 'vendor/pdfbox-app-3.0.8.jar:out/classes', 'Tables', join(IN, f), join(OUT, f)],
  ...
  console.log(name.padEnd(30), error ? `ERROR ${error}`
    : `tables=${r.tables} noContrast=${r.skippedNoContrast} colHeaders=${String(r.colHeaderCells).padStart(2)} rowHeaders=${String(r.rowHeaderCells).padStart(2)} th=${r.th}`);
}
writeFileSync(join(OUT, 'run.json'), JSON.stringify(rows, null, 2));
console.log(`\npromoted to /TH: ${rows.reduce((a, r) => a + (r.th ?? 0), 0)}`);
