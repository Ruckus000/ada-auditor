...
for (const file of files) {
  ...
  if (!result.ok) {
    ...
    // Planted wins when the key carries it; a key WITHOUT planted (a
    // refusal-expected stratum that delivered anyway) still has the truth
    // file, and grading such a delivery against null manufactured the run's
    // one false invented-claim. Truth is the fallback, never silence.
    const srcLang = key?.planted ? key.planted.language ?? null : truth ? truth.language : undefined;
