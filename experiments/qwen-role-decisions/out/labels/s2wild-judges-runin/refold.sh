...
# Run from experiments/qwen-role-decisions. Inputs: the per-round *-changed.json and
# *-replacements.jsonl beside this script (the 53 judged rows from
# s2wild-runin-consensus.jsonl plus null rows for the 23 uncovered changed cards),
# out/labels/s2wild-{v2,r3}-source-runin.jsonl, and a sidecar view per round in which
# the 11 fired documents point at out/suggest/wild-v4-runin. Writes to $OUT only.
set -e
