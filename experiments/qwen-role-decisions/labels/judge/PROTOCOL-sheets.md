...
2. Decide the ISO type of the boxed text using the definition:
   ...
   Rules: typography alone never decides (bold ≠ heading); a bold lead-in ending in a colon followed by a list on the same line is `P`; a box that covers only an enumerator (`A.`, `II.`, `3.`) is `Lbl`, whatever it enumerates — the heading is the card that carries the words; a card that carries an enumerator and heading words on one short line is `H`; a card whose only words are a division word and its number (`Section 2.`, `Article II`, `Chapter 3`), standing alone on its line above the text it divides, is `H` (v2.1, 2026-09-24) — only a bare enumerator with no word is `Lbl`; a head card split from a list item (id ending in `h`) is judged on its own line only, and its body card is judged as body text unless it is itself a heading; a heading merged with its first sentence is still `H`; `in_table_box` is geometric — a heading sitting above a table is still `H`.
