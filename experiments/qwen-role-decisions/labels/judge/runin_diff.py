...
Run from experiments/qwen-role-decisions. Writes out/suggest/wild-v4-runin/changed.json in the
v2_prep.py shape ({stem: {"new": [...], "changed": [...]}}) and diff-summary.json: new head
cards, same-id bodies whose text changed, removed ids, same-text neighbours whose prev/next
changed (and which of those flipped decision), and the must-judge bill split by coverage
(score >= THRESHOLD). Each document's old run is the one its labels were folded from.
"""
import json, pathlib
